...
记录和标签先保存到当前浏览器。保存时默认勾选提交到社区，可取消；云端公开比赛字段，昵称、备注不公开，标签和标记不上传。存储失败明确提示，表单留在原处；
...
数据库版本 2 增加 `outbox` 与 `nicknames`；记录和上传任务同事务写入。删除时默认排队撤回云端，先写入删除任务再移除本地记录。网络恢复或重新打开网站后自动同步，不上传此前未选择提交的记录。首次上传者有删除权，判重者不能删除他人记录。服务端每天每 IP 最多新增 5 局，达到额度后不跨天补传。旧版本本地记录可继续查看。
