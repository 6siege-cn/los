...
const draft=createDraft(rules.standard,operators);while(!draft.snapshot().complete){const candidates=operators.filter(op=>draft.canChoose(op.id));draft.choose(candidates[crypto.getRandomValues(new Uint32Array(1))[0]%candidates.length].id);}
...
try{
  ...
  const scheduled=await fetch(base+'/__scheduled');assert.equal(scheduled.status,200);
