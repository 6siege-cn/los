...
export function aggregateSnapshot(snapshot,{mapId='',ruleId='',mode='',matchType='normal',version='',family=false}={}){
  ...
  const rows=new Map(),maps=new Map();let total=0,attackWins=0;
  for(const op of catalog){const key=family?familyKey(op):op.id;if(!rows.has(key))rows.set(key,{...op,id:key,version:family?'family':op.version,picks:0,wins:0,bans:0,eligible:0});}
  for(const bucket of snapshot.buckets){
    const [map,rule,gameMode,type,alt,diy]=bucket.key;
    if((mapId&&map!==mapId)||(ruleId&&rule!==ruleId)||(mode&&gameMode!==mode)||(matchType!=='all'&&type!==matchType))continue;
    total+=bucket.total;attackWins+=bucket.attackWins;
    ...
    const eligible=new Set(catalog.filter(op=>op.version==='off'||(op.version==='alt'&&alt)||(op.version==='diy'&&diy)).map(op=>family?familyKey(op):op.id));
    for(const key of eligible)rows.get(key).eligible+=bucket.total;
  ...
  return {total,attackWins,defenseWins:total-attackWins,attackWinRate:rate(attackWins,total),
    operators:[...rows.values()].map(r=>({...r,winRate:rate(r.wins,r.picks),pickRate:rate(r.picks,r.eligible),banRate:rate(r.bans,r.eligible),bpRate:rate(r.picks+r.bans,r.eligible)})),
