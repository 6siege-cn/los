export const CACHE_KEY='six-siege:statistics-snapshot:v2';
export const FRESH_MS=30*60*1000,COOLDOWN_MS=5*60*1000;
export function validSnapshot(value){const count=n=>Number.isSafeInteger(n)&&n>=0;return value?.schemaVersion===2&&count(value.revision)&&Number.isFinite(Date.parse(value.generatedAt))&&Number.isFinite(Date.parse(value.checkedAt))&&Array.isArray(value.catalog)&&value.catalog.every(op=>op&&typeof op.id==='string'&&typeof op.name==='string'&&typeof op.avatar==='string'&&['attack','defense'].includes(op.side)&&['off','alt','diy','legacy'].includes(op.version))&&Array.isArray(value.buckets)&&value.buckets.every(b=>b&&Array.isArray(b.key)&&[6,8].includes(b.key.length)&&count(b.total)&&count(b.attackWins)&&b.attackWins<=b.total&&b.operators&&typeof b.operators==='object'&&Object.values(b.operators).every(c=>Array.isArray(c)&&c.length===3&&c.every(count)));}
